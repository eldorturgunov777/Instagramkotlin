...
class FeedAdapter(var context: Context, var items: ArrayList<Feed>) :
    ...
    companion object {
        ...
        private val TYPE_ITEM_ADS = 2
    }

    override fun getItemViewType(position: Int): Int {
        if (isHeader(position))
            return TYPE_ITEM_ADS
        return if (items[position].stories.size > 0)
            TYPE_ITEM_STORY
        else TYPE_ITEM_POST
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == TYPE_ITEM_ADS) {
            ...
            return FeedAdsViewHolder(header)
        }
        val view =
    ...
    class FeedAdsViewHolder(view: View) : RecyclerView.ViewHolder(view)
